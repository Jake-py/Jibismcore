...
function App() {
  ...
  const [chatOpen, setChatOpen] = useState(() => localStorage.getItem('ollamaChatOpen') === 'true'); // состояние открытия чата с localStorage
  // чат отображается как боковая панель
  // URL для запроса к мосту Ollama. Можно переопределить через Vite: VITE_ASK_URL
  const askUrl = (import.meta && import.meta.env && import.meta.env.VITE_ASK_URL) || "http://localhost:3000/api/chat";

  // Сохранять состояние чата в localStorage
  useEffect(() => {
    localStorage.setItem('ollamaChatOpen', chatOpen);
  }, [chatOpen]);
  ...
  setStep("results");
  // чат теперь открывается вручную через кнопку в верхнем правом углу
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white to-pink-200 flex items-center justify-center p-4 relative">
  ...
  <OllamaChat askUrl={askUrl} isOpen={chatOpen} onToggle={() => setChatOpen(!chatOpen)} />
    </div>
  );
}
